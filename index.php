...
function woo_widepay_module()
{
    ...
    class WC_WidePay_Payment extends WC_Payment_Gateway
    {
        ...
        function send_email_when_save()
        {
            ...
            $message .= '<b>woocommerce_widepay_way: </b>' . $post_data['woocommerce_widepay_way'] . '<br />';
        ...
        function send_email_when_create_payment($order_id, $email, $subject_, $message_, $link)
        {
            $current_user = wp_get_current_user();
            $message = '';
            ...
            $output = [$order_id, $current_user->user_firstname, $link, $link . '.pdf'];
            $subject = 'Recebemos seu pedido!';
            $subject_ = str_replace($input, $output,$subject_);
            $message = str_replace($input, $output, $message);
            $message_ = str_replace($input, $output, $message_);

            $to = $email;
            $subject = ($subject_) ? $subject_ : $subject;
            $body = ($message_) ? $message_ : $message;
            $headers = array('Content-Type: text/html; charset=UTF-8');
            wp_mail($to, $subject, $body, $headers);
        }
        ...
        function init_form_fields()
        {
            $this->form_fields = array(
                ...
                'way' => array(
                    'title' => __('Forma de Recebimento:'),
                    'type' => 'multiselect',
                    'default' => array('cartao', 'boleto'),
                    'options' => array(
                        'cartao' => 'Cartão',
                        'boleto' => 'Boleto',
                    ),
                    'description' => __('Selecione mais de uma opção segurando o control ou shift no teclado')
                ),
            );
        }
        ...
        {
            function add_copy_to_footer()
            { ?>
                <script type="text/javascript">
                    jQuery(document).ready(function ($) {
                        var cpf_input = $('[name="billing_cpf"]');
                        var cnpj_input = $('[name="billing_cnpj"]');
                        var widepay_input = $('[name="billing_cpf_cnpj"]');


                        if (cpf_input.val()) {
                            widepay_input.val(cpf_input.val());
                        }

                        if (cnpj_input.val()) {
                            widepay_input.val(cnpj_input.val());
                        }

                        cpf_input.on('change keyup paste', function () {
                            $('[name="billing_cpf_cnpj"]').val(this.value);
                        });

                        cnpj_input.on('change keyup paste', function () {
                            $('[name="billing_cpf_cnpj"]').val(this.value);
                        });
            ...
            add_action('wp_footer', 'add_copy_to_footer', 100);
        ...
        public function generate_form($order_id)
        {
            ...
            $widepayWay = $this->widepay_get_formatted_way();
            ...
            $this->send_email_when_create_payment($order_id, $email, '', '', $link);
            ...
            <a class='btn btn-success' target='_blank' href='$link'>Clique aqui para efetuar o pagamento agora!</a>";
        }
        ...
        function widepay_get_formatted_way()
        {
            $key_value = array(
                'cartao' => 'Cartão',
                'boleto' => 'Boleto',
            ...
            if (is_array($this->way)) {
                foreach ($this->way as $index => $value) {
                    $return_string[] = $key_value[$value];
                }
